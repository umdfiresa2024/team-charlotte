...
holidays_data <- holidays_data %>% filter(date >= as.Date("2003-11-24") & date <= as.Date("2011-11-24")) %>% mutate(formatted_date = date) %>% distinct(formatted_date)
...
holidays_data <- read.csv("/Users/paditya9/teamCharlotte/refined_holidays_data.csv", header = TRUE, stringsAsFactors = FALSE)[, c("formatted_date")]
...
station_pm_met_holiday_dataCombined <- station_pm_met_dataCombined %>% left_join(refined_holidayData, by = "formatted_date", keep = FALSE, unmatched = "drop")
