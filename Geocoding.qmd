...
station_pm_met_dataCombined <- station_pm_met_dataCombined[order(station_pm_met_dataCombined$formatted_date, station_pm_met_dataCombined$station_ID), ]

# %B is used for abbrevation for Month 
station_pm_met_dataCombined <- station_pm_met_dataCombined %>%
  mutate(month = format(as.Date(station_pm_met_dataCombined$formatted_date, format = "%Y-%m-%d"), "%B")) %>% 
  mutate (day_of_week = weekdays(as.Date(station_pm_met_dataCombined$formatted_date, format = "%Y-%m-%d")))
