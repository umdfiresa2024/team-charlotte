...
ggplot(df3, aes(x = date, y = mean_pm25, color = before_after))+
  ...
  geom_rect(aes(xmin = constructionstart,xmax = opendate, ymin = -Inf, ymax = Inf), alpha = 0.0025, fill = "yellow") +
...
breaks_interval <- c(-30.1, -29.4, -28.7, -28.1, -27.4, -26.7, -26.1, -25.4, -24.7, -24.0, -23.4)

# Create color palette with different shades of green (adjust green range as desired)
color_palette <- colorRampPalette(c("lightgreen", "darkgreen"))
colors <- color_palette(length(breaks_interval) - 1)


osmpos <- create_provider(name = "Thunderforest.Landscape",
...
# Creating Buffer for Map
extent <- buffer(x, width = 500)

bg <- get_tiles(ext(extent), zoom = 11, provider = osmpos)
...
# pch=19 gives filled circles
points(x, col = "blue", pch = 19, cex = 2)

# pch=17 gives filled triangles
points(pm_sources, col = "purple", pch = 17, cex = 2)
