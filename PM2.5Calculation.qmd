...
```{r}
write.csv(pm_addrs.geo, "pm_geocoded_data2.csv", row.names = FALSE)
```

```{r}
pm_addrs.geo <- read.csv("pm_geocoded_data.csv")
pm_new_addr <- pm_addrs.geo %>% 
  mutate(
    lat2 = ifelse(
...
```{r}
write.csv(pm_new_addr, "new_pm_geocoded_data2.csv", row.names = FALSE)
```

```{r}
pm_latlon <- cbind(pm_new_addr$lon2, pm_new_addr$lat2)
pm_latlon
```
...
```{r}
writeVector(x, "new_pm_sources.shp")
```
