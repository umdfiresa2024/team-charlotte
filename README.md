...
holidays_data1 <- holidays_data %>% filter(date >= as.Date("2003-11-24") & date <= as.Date("2011-11-24")) %>% mutate(formatted_date = date) %>% distinct(formatted_date)

# First-time users: Do NOT overwrite the CSV file.
# write.csv(holidays_data1, "refined_holidays_data.csv", row.names = F)
```
...
holidays_data <- read.csv("/Users/paditya9/teamCharlotte/refined_holidays_data.csv", header = TRUE, stringsAsFactors = FALSE)
...
# Over-writing into exsiting CSV file
# First-time users: Do NOT overwrite the CSV file.
