...
race<-get_acs(geography = "tract", variables=targetvars, state="NC", county="Mecklenburg", output="wide", year = 2010)
...
```{r}
racename<-race %>%
  rename(total = B19001_001, 
       less_than_10k = B19001_002, 
       `10k_to_15k` = B19001_003, 
       `15k_to_20k` = B19001_004, 
       `20k_to_25k` = B19001_005, 
       `25k_to_30k` = B19001_006, 
       `30k_to_35k` = B19001_007, 
       `35k_to_40k` = B19001_008, 
       `40k_to_45k` = B19001_009, 
       `45k_to_50k` = B19001_010, 
       `50k_to_60k` = B19001_011, 
       `60k_to_75k` = B19001_012, 
       `75k_to_100k` = B19001_013, 
       `100k_to_125k` = B19001_014, 
       `125k_to_150k` = B19001_015, 
       `150k_to_200k` = B19001_016, 
...
intdf<-as.data.frame(int)%>%
  ...
  mutate(pct_white=white*100/total, 
         ...
         pct_hisp=hisp*100/total)%>% mutate(FID = i)
...
sum_demog<-output %>%
  summarize(total = sum(total),
            ...
            asian = sum(asian),
            hisp = sum(hisp))%>%
  mutate(pct_white = white*100/total,pct_black = black*100/total,pct_native = native*100/total,pct_asian = asian*100/total,pct_hisp = hisp*100/total)
...
reduction_benefit <- everything%>%
  mutate(total_change = total*coef,white_change = white*coef,black_change = black*coef,
         asian_change = asian*coef,native_change = native*coef,hisp_change = hisp*coef)%>%
  summarize(total_change_sum=sum(total_change),
            ...
            asian_change_sum = sum(asian_change),
    ...
    mutate(total_change_ave=total_change_sum*100/total,
         ...
         asian_change_ave = asian_change_sum*100/asian)



final_result <- reduction_benefit %>%
  select(total_change_ave,black_change_ave,white_change_ave,native_change_ave,asian_change_ave)


kable(final_result,digits = 2)

#all racial groups seemed to get a similar amount of benefit from pollution reduction, ranging with whites getting -26.16 and asians getting -24.9
```
